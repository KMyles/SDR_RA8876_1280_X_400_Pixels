...
extern volatile uint32_t Frequency; //Sets a Default Frequency
extern volatile uint32_t Step;         // Sets the initial step  
extern void SetFrequency();
extern void setStep();
extern void setMode();
extern uint32_t F_Upper;
extern uint32_t F_Lpper;

//Screen address for GT911
uint8_t addr  = 0x5d;  //CTP IIC ADDRESS
const int GT911_RESET = 0;   //CTP RESET
const int GT911_INT =   1;   //CTP  INT
...
void checkfortouchscreen() {
  ...
  if (!st)  //Hardware touch interrupt
  {
    ...
    */
    ...
    if (count) {
      ...
      if (millis() > captouchbounce) { //cuts out multitouch
        captouchbounce = millis() + 550;//250 is the debounce, lower makes more sensitive but more false touches
   ...
   /***********************Set up touchscreen Areas*****************/
  /**************************band UP********************************/
          if((captouchx<100)&&(captouchy>350))  
          {
            ...
            else
            {
              Frequency=Frequency+Step;
                ...
                Serial.println("Band Up");
                Frequency=Frequency+1000000;
                SetFrequency();
          }
/************************BAND DOWN*******************************************/
          if((captouchx>160&&captouchx<228)&&(captouchy>350))  
          {
            ...
            else
            {
              Frequency=Frequency-Step;
                ...
                Serial.println("Band Down");
                Frequency=Frequency-1000000;
                SetFrequency();
          }
/***********************MODE******************************************/
          if((captouchx>240&&captouchx<400)&&(captouchy>350))  
          {
                Serial.println("Mode");
                setMode(); 
          }
...
/*************************Tuning Step*******************************/
if((captouchx>600&&captouchx<700)&&(captouchy>350))  
          {
                Serial.println("TS");
                setStep();
          } 
